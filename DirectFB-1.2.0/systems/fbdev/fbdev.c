...
/* <lab126> */
	 ...
	 /* Ensure that even when the framebuffer is of 4-bit depth, the software stack believes that 
	    it is an 8-bit depth one. */
	 if (shared->orig_var.bits_per_pixel == 4) {
		 shared->orig_var.bits_per_pixel = 8;
		 shared->current_var.bits_per_pixel = 8;
		 shared->fix.line_length *= 2;
	 }
	
	 if (viewport_shift_env) {
		 int dfb_shift = atoi(viewport_shift_env);
		 dfb_fbdev->framebuffer_base = dfb_fbdev->framebuffer_base + dfb_shift * shared->orig_var.xres;
		 shared->orig_var.yres = shared->orig_var.yres - dfb_shift;
		 shared->orig_var.yres_virtual = shared->orig_var.yres_virtual - dfb_shift;
		 shared->current_var.yres = shared->current_var.yres - dfb_shift;
		 shared->current_var.yres_virtual = shared->current_var.yres_virtual - dfb_shift;
	 }
/* </lab126> */
...
DFBResult
system_set_mode( const VideoMode         *mode,
                    const CoreSurfaceConfig *config )
{
   ...
   /*  printf(  "%s( mode: %p, config: %p )\n", __FUNCTION__, mode, config );*/

     D_ASSERT( mode != NULL );

/* Retrieve fixed informations like video ram size, same as what system_initialize does*/
     if (ioctl( dfb_fbdev->fd, FBIOGET_FSCREENINFO, &shared->fix ) < 0) {
          D_PERROR( "DirectFB/FBDev: "
                    "Could not get fixed screen information!\n" );
          goto error;
...
/* <lab126> */
    ...
    /*  shared->fix.line_length = shared->orig_var.xres;*/
     dfb_fbdev_var_to_mode( &shared->current_var,
                            &shared->current_mode );

   /*  printf(  "%s( exiting mode: %p, xres=%d, yres=%d fb=%p line_length=%d)\n", __FUNCTION__, mode, mode->xres, mode->yres, dfb_fbdev->framebuffer_base, shared->fix.line_length);*/
     return DFB_OK;
error:
                   ...
                   CoreLayerRegionConfigFlags *failed )
{
     ...
     D_INFO( "FBDev/Mode: Setting %dx%d  videomode x=%d, y=%d\n", config->source.w, config->source.h ,   videomode->xres, videomode->yres);
...
printf("dfb_surface_flip: %.3fs\n", ((endTime.tv_sec-startTime.tv_sec)*1000 + (endTime.tv_usec - startTime.tv_usec)/1000)/1000.0);
