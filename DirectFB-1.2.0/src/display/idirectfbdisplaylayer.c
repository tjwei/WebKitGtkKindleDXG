...
/*
    ...
    memset(&we, 0, sizeof(DFBInputEvent));

    // Send an event only if necessary (note that the event only support one axis at a time 
    dfb_windowstack_get_cursor_position( data->stack, &oldX, &oldY );
    we.type = DIET_AXISMOTION;
    we.flags |= DIEF_AXISABS;
    if (oldX != x) {
        we.axis = DIAI_X;
        we.axisabs = x;
        result = dfb_wm_process_input( data->stack, &we );
        if (DFB_OK != result) {
            return result;
        }
    }
