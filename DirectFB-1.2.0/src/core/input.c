...
//     printf(  "DFB  -> KEYCODE    %d\n",        event->key_code );
//     printf(  "DFB  -> KEYID      0x%04x\n",    event->key_id );
//     printf(  "DFB  -> KEYSYMBOL  0x%04x\n",    event->key_symbol );
     /*
      * 1. Fixup event...
      */
     ...
     switch (event->type) {
          ...
          case DIET_KEYPRESS:
          case DIET_KEYRELEASE:
//               printf("in DFB DIET_KEYPRESS/DIET_KEYRELEASE\n");
                  ...
                  int              code )
{
     ...
     if (entry->code != code) {
          ...
          if (entry->identifier != DIKI_UNKNOWN) {
               entry->symbols[DIKSI_BASE] = id_to_symbol( entry->identifier, 0, 0);
               entry->symbols[DIKSI_BASE_SHIFT] = id_to_symbol( entry->identifier, DIMM_SHIFT, 0);
               entry->symbols[DIKSI_ALT] = id_to_symbol( entry->identifier, DIMM_ALTGR, 0);
               entry->symbols[DIKSI_ALT_SHIFT] = id_to_symbol( entry->identifier, (DIMM_SHIFT | DIMM_ALTGR), 0);
               return entry;
          } else if (entry->symbols[DIKSI_BASE] == DIKS_NULL) {
               return NULL;
          }
          /* </lab126> */
...
// <lab126>
void doMyMap(DFBInputEvent      *event) {
        switch (event->key_code) {
            // MARIO Keycode mappings
            case 139: // 'MENU' key
            case 91: // 'BACK' key
            case 90: // 'Aa' key
            case 94: // 'SYM' key
            // LUIGI Keycode mappings which are different from Mario.
            // Note: the keys do NOT collide.
            case 158: // 'BACK' key
            case 190: // 'Aa' Key  //K2=90 K3=190
            case 126: // 'SYM' Key  //K2=94 K3=126
            case 191: // right hand side next page
            case 193: // left hand side prev page
                event->key_id = 0xf669;
                break;
            default:
                break;
	...
	switch (event->key_id) {
            case 0xf65d: // MARIO 'HOME' key
               event->key_id = DIKI_HOME;
               event->key_symbol = DIKS_HOME;
               break;
            case 0xf64e: // MARIO left hand side, LUIGI right hand side prev page
               event->key_code = 104; // Set to Page Up keycode
               event->key_id = DIKI_PAGE_UP;
               event->key_symbol = DIKS_PAGE_UP;
               break;
            case 0xf64d: // left hand side next page
               if (event->key_code == 104) { // 104 is Page Up, yikes!
                 event->key_code = 109; // Set to Page Down keycode
                 event->key_id = DIKI_PAGE_DOWN;
                 event->key_symbol = DIKS_PAGE_DOWN;
               }
               break;
            case 0xf669:
               if (event->key_code == 124 || // MARIO right hand side next page
                   event->key_code == 191) { // LUIGI right hand side next page
                 event->key_code = 109; // Set to Page Down keycode
                 event->key_id = DIKI_PAGE_DOWN;
                 event->key_symbol = DIKS_PAGE_DOWN;
               } else if (event->key_code == 193) { // LUIGI left hand side prev page
                 event->key_code = 104; // Set to Page Up keycode
                 event->key_id = DIKI_PAGE_UP;
                 event->key_symbol = DIKS_PAGE_UP;
               } else if (event->key_code == 139) { // 'MENU' key
                 event->key_symbol = DIKS_MENU;
               } else if (event->key_code == 91 ||  // MARIO 'BACK' key
                          event->key_code == 158) { // LUIGI 'BACK' key
                 event->key_symbol = DIKS_BACK;
               } else if  (event->key_code == 90 ||  // MARIO 'Aa' key
                           event->key_code == 190) { // LUIGI 'Aa' key
                 event->key_symbol = DIKS_CONTROL; // no associated symbol 
               } else if  (event->key_code == 94 ||  // MARIO 'SYM' key
                           event->key_code == 126) { // LUIGI 'SYM' key
                 event->key_symbol = DIKS_TAB; // no associated symbol
               }
               break;
            case 0xf631:
               event->key_id = DIKI_SHIFT_L;
               event->key_symbol = DIKS_SHIFT;
               break;
            case DIKI_SPACE:
               event->key_symbol = DIKS_SPACE;
               break;
            case DIKI_ENTER:
