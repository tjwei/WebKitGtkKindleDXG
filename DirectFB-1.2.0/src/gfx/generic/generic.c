...
static DirectFB_AddDirtyRectangle_Fct g_pfnKindleAddDirtyRectangle = 0;
static u8 *g_fb = 0;
static u8 *g_virtualFb = 0;
...
{
    g_pfnKindleAddDirtyRectangle = fct;
}

extern DirectFB_AddDirtyRectangle_Fct kindle_get_dirty_rectangle_cb() 
{
    return g_pfnKindleAddDirtyRectangle;
}

/* Return false if the pointers are not available just yet */
extern DFBResult kindle_get_framebuffer(u8 **fb, u8 **virtualFb, int *width, int *height) 
{
    *fb = g_fb; 
    *virtualFb = g_virtualFb;
    *width = g_fbWidth; 
    *height = g_fbHeight;
    return (g_fb) ? DFB_OK : DFB_TEMPUNAVAIL;
}
/* </lab126> */
...
void gBlit( CardState *state, DFBRectangle *rect, int dx, int dy )
{
	 ...
	 // Chris <lab126>
/*	 if (gfxs->src_bpp == 4) {
		 printf("Blitting: \nsrc: %d, pitch: %d, format: %s \ndst: %d, pitch: %d, format: %s\nl: %d x: %d y: %d w:%d h: %d\n", gfxs->src_bpp, gfxs->src_pitch, dfb_pixelformat_name(gfxs->src_format), gfxs->dst_bpp, gfxs->dst_pitch, dfb_pixelformat_name(gfxs->dst_format), gfxs->length, rect->x, rect->y, rect->w, rect->h);
	 } else if (gfxs->dst_bpp == 4) {
		 printf("Blitting: \nsrc: %d, pitch: %d, format: %s \ndst: %d, pitch: %d, format: %s\nl: %d x: %d y: %d w:%d h: %d\n", gfxs->src_bpp, gfxs->src_pitch, dfb_pixelformat_name(gfxs->src_format), gfxs->dst_bpp, gfxs->dst_pitch, dfb_pixelformat_name(gfxs->dst_format), gfxs->length, rect->x, rect->y, rect->w, rect->h);
	 }
*/
     if (dfb_config->software_warn) {
...
/* <lab126> */
            // If the conversion is from ARGB to LUT8, then it is a blit to the frame buffer
            // Using the generic way of processing is very expensive, so make it a special case
            if (gfxs->src_format == DSPF_ARGB && gfxs->dst_format == DSPF_LUT8) {

                // Right now the alpha is only set if it comes from the cursor. (in the case of the cursor, the alpha is either 0 or 1)
                // Later it may change, so this code may have to be modified to support alpha correctly
                for (h = rect->h; h; h--) {
                    u32 *src = gfxs->Bop[0];
                    u8 *dst = gfxs->Aop[0];
                    for (x=0; x<rect->w; x++) {
            ...
            // Else if blitting from the LUT8 to LUT8, no need to worry about alpha, etc. So just do a memcpy
            } else if (gfxs->src_format == DSPF_LUT8 && gfxs->dst_format == DSPF_LUT8) {

                // CAUTION: This is a hack... To catch the pointers of the framebuffer and the virtual buffer, we rely on their pitch becasue they are of the same size.
                if (gfxs->dst_pitch == gfxs->src_pitch && gfxs->dst_height == gfxs->src_height)
                {

                    if ( ( g_fbWidth == gfxs->dst_height && g_fbHeight == gfxs->dst_pitch ) || (g_fb != gfxs->dst_org[0]) ){
                        g_fbWidth = gfxs->dst_pitch;
                        g_fbHeight = gfxs->dst_height;
                        g_fb = (u8 *) gfxs->dst_org[0];
                        g_virtualFb = (u8 *) gfxs->src_org[0];
                    }

                }

                // If blitting is done from the virtual to the framebuffer and notification is requested, just notify, otherwise perform the blit
                if (g_fb == (u8 *) gfxs->dst_org[0] && g_virtualFb == (u8 *) gfxs->src_org[0] && g_pfnKindleAddDirtyRectangle) {
                     g_pfnKindleAddDirtyRectangle(rect);
                // Optimize the case where only one memcopy can be done (usually on full screen update)
                } else if (gfxs->src_pitch == gfxs->dst_pitch && rect->w == gfxs->src_pitch) {
                    memcpy(gfxs->Aop[0], gfxs->Bop[0], rect->h * rect->w * sizeof(u8));
                } else {
                    // Else do it line by line
                    for (h = rect->h; h; h--) {
                        memcpy(gfxs->Aop[0], gfxs->Bop[0], rect->w * sizeof(u8));
                        Aop_next( gfxs );
                        Bop_next( gfxs );
                    }
                }
            ...
            //printf("-- Perf6: %d functs in %.6fs\n", i, 
            //	   ((endTime.tv_sec-startTime.tv_sec)*1000000 + (endTime.tv_usec - startTime.tv_usec))/1000000.0);
            // </lab126>
