...
again:
	...
	if (result < 0) {
/* <lab126> Log the reason why the hanshake failed (gnutls.h.in contains the error definitions) */
        gnutls_perror(result);

        // If the error is fatal, just mark it as an ssl error so that libsoup does not try to establish this connection again and again
        if (gnutls_error_is_fatal (result)) {
            g_set_error (err, SOUP_SSL_ERROR,
                     SOUP_SSL_ERROR_FATAL,
                     "Unable to handshake");
        } else {
...
//	if (gnutls_priority_set_direct (session, "NORMAL:!VERS-TLS1.1:!VERS-TLS1.0", NULL) != 0)
	if (gnutls_priority_set_direct (session, "NORMAL:!VERS-TLS1.1:!VERS-TLS1.2:!VERS-TLS1.0", NULL) != 0)
		goto THROW_CREATE_ERROR;
