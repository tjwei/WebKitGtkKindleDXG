...
/* <lab126> */
//extern float chris_time_from_start();
/* </lab126> */
...
typedef struct {
    ...
    /* <lab126> */
    gboolean     hasCertificateException;
    /* </lab126> */
...
typedef struct {
	...
	gboolean ssl_strict;
    /* <lab126> */
    SoupSSLCredentials *ssl_creds_cert_exception;
    /* </lab126> */
...
/* <lab126> */
#define SOUP_SESSION_MAX_CONNS_PER_PROXY_DEFAULT 6
/* </lab126> */
...
soup_session_init (SoupSession *session)
{
    ...
    /* <lab126> */
    priv->max_conns_per_proxy = SOUP_SESSION_MAX_CONNS_PER_PROXY_DEFAULT;
    /* </lab126> */
...
finalize (GObject *object)
{
    ...
    /* <lab126> */
    if (priv->ssl_creds_cert_exception)
        soup_ssl_free_client_credentials (priv->ssl_creds_cert_exception);
    /* </lab126> */
...
soup_session_class_init (SoupSessionClass *session_class)
{
    ...
    /* <lab126> */
    /**
     * SOUP_SESSION_MAX_CONNS_PER_PROXY:
     *
     * Alias for the #SoupSession:max-conns-per-proxy property.
     * (The maximum number of connections that the session can
     * open at once to the proxy.)
     **/
    g_object_class_install_property (
        object_class, PROP_MAX_CONNS_PER_PROXY,
        g_param_spec_int (SOUP_SESSION_MAX_CONNS_PER_PROXY,
                  "Max Per-Proxy Connection Count",
                  "The maximum number of connections that the session can open at once to the current proxy",
                  1,
                  G_MAXINT,
                  SOUP_SESSION_MAX_CONNS_PER_PROXY_DEFAULT,
                  G_PARAM_READWRITE));
    /* </lab126> */
	/**
	      ...
	      const GValue *value, GParamSpec *pspec)
{
	...
	case PROP_MAX_CONNS_PER_HOST:
		...
		break;
    /* <lab126> */
    case PROP_MAX_CONNS_PER_PROXY:
        priv->max_conns_per_proxy = g_value_get_int (value);
        break;
    /* </lab126> */
	case PROP_USE_NTLM:
	      ...
	      GValue *value, GParamSpec *pspec)
{
    ...
    /* <lab126> */
    case PROP_MAX_CONNS_PER_PROXY:
        g_value_set_int (value, priv->max_conns_per_proxy);
        break;
    /* </lab126> */    
	case PROP_USE_NTLM:
		feature = soup_session_get_feature (session, SOUP_TYPE_AUTH_MANAGER_NTLM);
...
static SoupSessionHost *
/* <lab126> */
get_host_for_uri (SoupSession *session, SoupURI *uri, SoupURI *proxy_uri)
/* </lab126> */
{
	SoupSessionPrivate *priv = SOUP_SESSION_GET_PRIVATE (session);
	SoupSessionHost *host;
/* <lab126> */
    /* In the case of ssl connections, the destination host should be used, because the connection to
      the proxy cannot and must not be shared with connections that go to other servers */
    if (proxy_uri && uri->scheme != SOUP_URI_SCHEME_HTTPS) {
        uri = proxy_uri;
    }
/* </lab126> */
...
/* <lab126> */
static SoupSessionHost *
get_host_for_message (SoupSession *session, SoupMessage *msg, SoupURI *proxy_uri)
{
	return get_host_for_uri (session, soup_message_get_uri (msg), proxy_uri);
}
/* </lab126> */
...
redirect_handler (SoupMessage *msg, gpointer user_data)
{
	...
	/* <lab126> 
	 * In a redirect, the message does not get unqueued then requeued,
	 * the message's uri just get set to the new uri. Therefore,
	 * the host num_messages for the old uri needs to be decremented here,
	 * and the host num_messages for the new uri needs to be incremented
	 * here.
	 */
	g_object_get(G_OBJECT(session), SOUP_SESSION_PROXY_URI, &proxy_uri, NULL);
	g_mutex_lock (priv->host_lock);

	host = get_host_for_message (session, msg, proxy_uri);
	/* CONNECT messages are not included in the host's num_messages.
	 * Therefore, don't decrement num_messages for CONNECT.
	 */
	if (msg->method != SOUP_METHOD_CONNECT && host->num_messages > 0)
		host->num_messages--;
	...
	host = get_host_for_message (session, msg, proxy_uri);
	/* CONNECT messages should not be included in the host's num_messages.
	 * Therefore, don't increment num_messages for CONNECT.
	 * See queue_message().
	 */
			      ...
			      SoupConnection *conn)
{
    ...
    /* <lab126> */
    /* Fixed libsoup bug:
        When a message gets cancelled in the previous emit, the connection is still considered in use and not left as an idle one */
    g_object_get(G_OBJECT(item->msg), SOUP_MESSAGE_STATUS_CODE, &msg_status, NULL);
    if (msg_status == SOUP_STATUS_CANCELLED) {
        // Ensure that the connection does not get closed because the message was not actually sent
        soup_connection_clear_unused_timeout(conn); // This is lab126 specific as we want the connection to remain opened...
				...
				SoupConnection *conn, guint status)
{
	...
	for (item = soup_message_queue_first (priv->queue); item; item = soup_message_queue_next (priv->queue, item)) {
		...
		if (SOUP_MESSAGE_IS_STARTING (msg) &&
		    get_host_for_message (session, msg, item->proxy_uri) == host)
			     ...
			     gboolean *try_pruning)
{
    ...
    /* <lab126> */
	host = get_host_for_message (session, item->msg, item->proxy_uri);
    if (soup_message_get_force_new_connection (item->msg)) {
        soup_message_set_force_new_connection (item->msg, FALSE);
    } else {
	 ...
	 */
    /* <lab126>: The limit should be the number of pending messages and not (num_messages / 2) */
	if (num_pending >= host->num_messages) {
    /* </lab126> */
		g_mutex_unlock (priv->host_lock);
		return NULL;
	}
    ...
    /* <lab126> */
    if (item->proxy_addr) {
        if (host->num_conns >= priv->max_conns_per_proxy) {
            g_mutex_unlock (priv->host_lock);
            return NULL;
        }
    } else if (host->num_conns >= priv->max_conns_per_host) {
		g_mutex_unlock (priv->host_lock);
		return NULL;
	}
    /* </lab126> */
	...
	if (uri->scheme == SOUP_URI_SCHEME_HTTPS) {
        ...
        if (host->hasCertificateException && !priv->ssl_creds_cert_exception)
            priv->ssl_creds_cert_exception = soup_ssl_get_client_credentials (NULL);

/* <lab126> */
        /* Add the ssl credentials only if there is no exception for this host */
		ssl_creds = host->hasCertificateException ? priv->ssl_creds_cert_exception : priv->ssl_creds;
/* </lab126> */
...
message_finished (SoupMessage *msg, gpointer user_data)
{
	...
	if (!SOUP_MESSAGE_IS_STARTING (msg)) {
		...
		host = get_host_for_message (session, item->msg, item->proxy_uri);
		/* CONNECT messages are not included in the host's num_messages.
		 * Therefore, should not decrement num_messages for CONNECT.
		 * See queue_message().
		 */
		if (msg->method != SOUP_METHOD_CONNECT && host->num_messages > 0)
			host->num_messages--;
	       ...
	       SoupSessionCallback callback, gpointer user_data)
{
	...
	/* <lab126> */
	g_object_get(G_OBJECT(session), SOUP_SESSION_PROXY_URI, &proxy_uri, NULL);
	host = get_host_for_message (session, item->msg, proxy_uri);
	/* CONNECT messages use already established connections, so they
	 * do not count toward a host's num_messages, which is used for
	 * determining if a new connection to the host should be made.
         */
	if (msg->method != SOUP_METHOD_CONNECT)
		host->num_messages++;
	/* </lab126> */
...
/* <lab126> */
void
soup_session_prepare_for_uri (SoupSession *session, SoupURI *uri, SoupURI *proxy_uri)
/* </lab126> */
{
	SoupSessionPrivate *priv = SOUP_SESSION_GET_PRIVATE (session);
	SoupSessionHost *host;
	...
	g_mutex_lock (priv->host_lock);
/* <lab126> */
	host = get_host_for_uri (session, uri, proxy_uri);
/* </lab126> */
...
/* <lab126> */
/**
 * soup_session_current_connection_count:
 * @session: a #SoupSession
 *
 * Return the number of opened connections for this session without any regards 
 * to the destination servers.
 *
 * This function is only applicable when a proxy is used. It is ignored otherwise
 *
 *
 * Since: Kindle extension
 **/
guint               soup_session_current_connection_count(SoupSession        *session) 
{
    SoupSessionPrivate *priv;
...
/**
 * soup_session_close_idle_connections:
 * @session: a #SoupSession
 *
 * Closes idle_conn_count idle connections. 
 * If idle_conn_count = -1, closes all idle connections.
 *
 *
 * Since: Kindle extension
 **/
void                soup_session_close_idle_connections   (SoupSession        *session,
                                 gint idle_conn_count)
{
    GSList *conns = NULL, *c;
    GHashTableIter iter;
    gpointer conn, host;
	...
	g_mutex_lock (priv->host_lock);
	g_hash_table_iter_init (&iter, priv->conns);
	while (g_hash_table_iter_next (&iter, &conn, &host) && idle_conn_count > 0) {
		state = soup_connection_get_state (conn);
		if (state == SOUP_CONNECTION_REMOTE_DISCONNECTED ||
		    (state == SOUP_CONNECTION_IDLE)) {
            conns = g_slist_prepend (conns, g_object_ref (conn));
            idle_conn_count--;
        }
	}
...
/**
 * soup_session_add_certificate_exception:
 * @session: a #SoupSession
 * @msg: a #SoupMessage that triggered an invalid certificate exception
 *
 * Marks the host as being trusted (only valid for ssl connections)
 *
 *
 * Since: Kindle extension
 **/
void                soup_session_add_certificate_exception   (SoupSession        *session,
                                 SoupMessage *msg)
{
    SoupSessionHost *host;
    SoupURI *uri;
    ...
    /* Means only something for ssl connections */
    uri = soup_message_get_uri (msg);
    if (uri->scheme != SOUP_URI_SCHEME_HTTPS) {
        return;
    }
    host = get_host_for_message(session, msg, NULL);
    host->hasCertificateException = TRUE;
}
/* </lab126> */
