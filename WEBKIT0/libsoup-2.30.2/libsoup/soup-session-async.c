...
/* <lab126> */
/*
static int gStartTimeInitialized = 0; 
static struct timespec gStartTime;
float chris_time_from_start()
{
	struct timespec end;
	clock_gettime(CLOCK_MONOTONIC, &end);
    return end.tv_sec-gStartTime.tv_sec + (end.tv_nsec-gStartTime.tv_nsec)/1000000000.0;
}
*/
/* </lab126> */
...
/* <lab126> a double resolution was previously done: 
   the resolver computed the address from the uri, but when this callback function
   was called, it converted the SoupAddress to a SoupURI, therefore forcing this method to resolve the address once
   again (callback was resolved_proxy_addr) 
   */
static void
resolved_proxy_uri (SoupProxyURIResolver *proxy_resolver,
		    guint status, SoupURI *proxy_uri, SoupAddress *proxy_addr, gpointer user_data)
{
	SoupMessageQueueItem *item = user_data;
	SoupSession *session = item->session;
	...
	if (proxy_uri) {
        ...
        if (proxy_addr) {
            resolved_proxy_addr(proxy_addr, status, user_data);
        } else {
...
got_connection (SoupConnection *conn, guint status, gpointer session)
{
	...
	if (tunnel_addr) {
        ...
        /* <lab126>
         * At this point, the connection is in the IDLE state,
         * which will get set to IN_USE when the HTTP CONNECT
         * request is about to be sent.
         * For SSL, the connection should remain in the
         * CONNECTING state until the HTTP CONNECT request
         * succeeds.
         */
...
run_queue (SoupSessionAsync *sa)
{
    ...
    /* <lab126> */
    /* Chris to remove once performance work is done */
/*    if (!gStartTimeInitialized) {
        clock_gettime(CLOCK_MONOTONIC, &gStartTime);
        gStartTimeInitialized = 1;
    }
*/

    /* And if we are not currently running the queue. This function can been called
      asynchronously or synchronously depending on whether the data was cached or not */
    static gboolean in_run_queue = FALSE;
    static gboolean run_queue_again = FALSE;
    if (in_run_queue) {
        run_queue_again = TRUE;
        return;
    }
    in_run_queue = TRUE;
    /* </lab126> */
 ...
 try_again:
/*    printf("run_queue %d\n", soup_session_current_connection_count(session)); */
	for (item = soup_message_queue_first (queue);
		...
		if (!item->resolved_proxy_addr) {
			...
			if (proxy_resolver) {
                ...
                /* <lab126> */
                /* If the address was cached, it may already be resolved otherwise wait for it to be resolved */
                if (!item->resolved_proxy_addr) 
                    continue;
                /* </lab126> */
			} else
