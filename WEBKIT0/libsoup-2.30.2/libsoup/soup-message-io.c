...
/* <lab126> Give a chance to the ssl error to get reported */
    if (g_error_matches (error, SOUP_SSL_ERROR, SOUP_SSL_ERROR_CERTIFICATE)) {
        soup_message_set_status_full (msg, SOUP_STATUS_SSL_CERTIFICATE_FAILED, error->message);

        /* Queue again if the the message error has been overwritten by the set_status notify */
        guint msg_status;
        g_object_get(G_OBJECT(msg), SOUP_MESSAGE_STATUS_CODE, &msg_status, NULL);
        if (msg_status == SOUP_STATUS_OK) {
	...
	if (io->mode == SOUP_MESSAGE_IO_CLIENT &&
        ...
        (!error || error->domain != SOUP_SSL_ERROR) &&
