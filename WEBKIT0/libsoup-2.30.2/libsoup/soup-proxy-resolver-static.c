...
typedef struct {
    ...
    guint           proxy_status;
    gboolean        resolving_proxy_addr;
    gboolean        resolved_proxy_addr;
    /* </lab126> */
	      ...
	      const GValue *value, GParamSpec *pspec)
{
	...
	case PROP_PROXY_URI:
        ...
        priv->resolving_proxy_addr = FALSE;
        priv->resolved_proxy_addr = FALSE;
        /* </lab126> */
		break;
	default:
...
resolved_address (SoupAddress *addr, guint status, gpointer data)
{
    ...
    if (priv->proxy_addr != addr && status == SOUP_STATUS_OK) {
        if (priv->proxy_addr)
            g_object_unref (priv->proxy_addr);
        priv->resolving_proxy_addr = FALSE;
        priv->resolved_proxy_addr = TRUE;
        priv->proxy_addr = addr ? g_object_ref(addr) : NULL;
        priv->proxy_status = status;
    }
    /* </lab126> */
		 ...
		 gpointer            user_data)
{
    ...
    /* <lab126> */
    /* If the address was already resolved and cached, just use it */
    if (priv->resolved_proxy_addr) {
        // resolved_address unrefs automatically the address passed, so ensure that the counter is incremented first
        g_object_ref (priv->proxy_addr); 
        resolved_address(priv->proxy_addr, priv->proxy_status, ssad);
    }

    /* Else if it was in the process of being resolved, do not do anything. The run_queue will any way process the messages as soon as it gets resolved */
    else if (priv->resolving_proxy_addr)
        ; /* Do nothing */
        ...
        soup_address_resolve_async (ssad->addr, async_context,
                                    ...
                                    ssad);
        priv->resolving_proxy_addr = TRUE;
    }
    /* </lab126> */
}
