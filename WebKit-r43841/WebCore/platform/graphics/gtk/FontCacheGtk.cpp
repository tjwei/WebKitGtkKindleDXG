...
const SimpleFontData* FontCache::getFontDataForCharacters(const Font& font, const UChar* characters, int length)
{
    ...
    if (!prim->m_fallbacks) {
        // Do not use the font's pattern directly for fallback. Should only use
        // the family, weight, slant, and pixel-size properties, which are the
        // same set of properties used in FontPlatformDataGtk when looking up
        // a font, then do config-substitute and default-substitute on the
        // pattern.
        FcChar8* family;
        if (FcPatternGetString(prim->m_pattern, FC_FAMILY, 0, &family) == FcResultMatch) {
            int weight;
            int slant;
            double pixelSize;
            ...
            if (FcPatternGetInteger(prim->m_pattern, FC_WEIGHT, 0, &weight) == FcResultMatch)
                FcPatternAddInteger(pattern, FC_WEIGHT, weight);
            if (FcPatternGetInteger(prim->m_pattern, FC_SLANT, 0, &slant) == FcResultMatch)
                FcPatternAddInteger(pattern, FC_SLANT, slant);
            if (FcPatternGetDouble(prim->m_pattern, FC_PIXEL_SIZE, 0, &pixelSize) == FcResultMatch)
                FcPatternAddDouble(pattern, FC_PIXEL_SIZE, pixelSize);

            FcConfigSubstitute(NULL, pattern, FcMatchPattern);
            FcDefaultSubstitute(pattern);
            prim->m_fallbacks = FcFontSort(NULL, pattern, FcTrue, NULL, &fresult);

            FcPatternDestroy(pattern);
        } else {
            // Should not arrive here, but if it did, do what the original code
            // does.
            prim->m_fallbacks = FcFontSort(NULL, prim->m_pattern, FcTrue, NULL, &fresult);
        }
    }
    ...
    if (!prim->m_fallbacks) {
        // Just in case of weird errors in fontconfig.
        return 0;
    }
    // </lab126>
