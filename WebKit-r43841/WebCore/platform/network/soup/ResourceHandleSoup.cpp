...
// <lab126>: only do content sniffing when no Content-type is specified
...
static void ensureSessionIsInitialized(SoupSession* session)
{
    ...
    // <lab126>
    char *env = getenv("WEBKIT_CONTENT_ENCODING");
    if (env && *env != '\0') {
        // Applied patch to support gzip/deflate encoding
...
bool ResourceHandle::startHttp(String urlString)
{
    ...
    // <lab126>
    if (!soup_session_get_feature(session, SOUP_TYPE_CONTENT_DECODER)) {
        // No encoding.
...
// <lab126>
// Retrieve the number of allowed concurrent connections in a given session directly from libsoup
unsigned initializeMaximumHTTPConnectionCountPerHost()
{
#define DEFAULT_CONN_COUNT 8
    static unsigned preferredConnectionCount = DEFAULT_CONN_COUNT;
    char *env = getenv("WEBKIT_MAX_HTTP_CONNECTIONS");
    if (env) {
        preferredConnectionCount = atoi(env);
        if (preferredConnectionCount <= 0) {
            preferredConnectionCount = DEFAULT_CONN_COUNT;
        }
    }
