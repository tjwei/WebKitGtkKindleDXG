...
// <lab126>
			KINDLE_BEGIN(Kindle_Debug_Resources)
				clock_gettime(CLOCK_MONOTONIC, &request->startLoadTime);
				KINDLE_PRINT("\033[35mLoading %s...\033[0m\n", request->cachedResource()->url().latin1().data());
			KINDLE_END()
// </lab126>
...
void Loader::Host::didFinishLoading(SubresourceLoader* loader)
{
	...
	KINDLE_BEGIN(Kindle_Debug_Resources)
		...
		KINDLE_PRINT("\033[35m%.3f - Loaded %s in %.3fs ...\033[0m\n", kindle_time_from_start(), resource->url().latin1().data(), kindle_time_diff(request->startLoadTime, endTime));
	KINDLE_END()
	// </lab126>
