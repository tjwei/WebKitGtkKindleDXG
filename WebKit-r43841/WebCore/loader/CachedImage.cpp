...
void CachedImage::data(PassRefPtr<SharedBuffer> data, bool allDataReceived)
{
    ...
    // <lab126>: The image should trigger a redraw only once all the data is received 
    // (to prevent multiple paints of incomplete image. By choice, we do not trigger a 
    // paint as soon as the size is known (indicating that layout can be computed).
    // ToDo: try to only notify observers the first time the size becomes available or when the data is fully available
    // ToDo: It would be nice to add a timer so that if it takes longer than 2 seconds (for example) to receive the full image, we just 
    // regularly update any way (works for very large images)
