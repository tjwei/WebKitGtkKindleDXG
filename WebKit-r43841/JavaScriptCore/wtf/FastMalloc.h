...
namespace WTF {
    ...
    WEBCORE_EXPORT void* fastMalloc(size_t n);
    ...
    WEBCORE_EXPORT void fastFree(void* p);
...
// <lab126>
#ifdef __EXCEPTIONS
WTF_PRIVATE_INLINE void* operator new(size_t s) throw (std::bad_alloc) { 
    void *p;
    if ((p = fastMalloc(s)) == NULL)
        throw std::bad_alloc();
    return p;
}
