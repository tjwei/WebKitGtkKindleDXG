					...
					cairo_scaled_font_t       **font_out)
{
	...
	fs_metrics.height =        DOUBLE_FROM_26_6(metrics->height) * y_factor;
        // <lab126>
	// HelveticaNeue font does not provide metrics data, must calculate the
	// data ourselves. This is done by calculating the max ascent/descent
	// of most alpha-numeric characters as done in microwindows. See
	// platform/cvm/pbp/microwin-kindle/src/engine/FreeType2FontRenderer.c,
	// specifically, the function FreeTypeFont_GetFontInfo/2.
	if (fs_metrics.ascent <= 0 || fs_metrics.descent <= 0 || fs_metrics.height <= 0) {
		...
		ascent = DOUBLE_FROM_26_6(bbox.yMax) * y_factor;
		descent = DOUBLE_FROM_26_6(-bbox.yMin) * y_factor;
				
		if (ascent > fs_metrics.ascent) {
		    fs_metrics.ascent = ascent;
		}

		if (descent > fs_metrics.descent) {
		    fs_metrics.descent = descent;
		}
	    }
	    height = fs_metrics.ascent + fs_metrics.descent;
	    if (fs_metrics.height < height) {
		fs_metrics.height = height;
	    }
	}
	// </lab126>
			     ...
			     cairo_scaled_glyph_info_t	 info)
{
	...
	if (glyph->format == FT_GLYPH_FORMAT_OUTLINE) {
          /* <lab126> Use render_glyph_bitmap to let freetype directly
	     * render the outline to a bitmap.
	     */
