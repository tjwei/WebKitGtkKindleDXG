...
gdk_window_class_init (GdkWindowObjectClass *klass)
{
  ...
  /**
   * GdkWindow::new_window: @window: the newly created window
   *
   * The ::new_window signal is emitted on the top level window
   * when a new window is created.
   *
   * Lab126 extension --- we need to figure out how this works for gtk-2.16 given the the marshalers has changed.
   */   

  signals[NEW_WINDOW] =
    g_signal_new (g_intern_static_string ("new_window"),
		  G_OBJECT_CLASS_TYPE (klass),
		  G_SIGNAL_RUN_LAST,
		  0,
		  NULL, NULL,
		  gdk_marshal_VOID__OBJECT_POINTER,
		  G_TYPE_NONE,
		  2,
		  GDK_TYPE_WINDOW,
		  G_TYPE_POINTER); 
		...
		gint           attributes_mask)
{
  ...
  /* <lab126> This code will have to move to make sure it doesnot break XORG build and take note of marshalers change */

#ifndef GDK_WINDOWING_X11   
  g_signal_emit_by_name(gdk_get_default_root_window (), "new_window", window, gdk_window_get_surface(window));
#endif
  return window;
  /* </lab126> */
}
