...
//  impl->drawable.paint_region   = NULL;
  ...
  /* <lab126> */
  /* Forcing that this will create the primary suface and primary window so that SetVideo will take effect. SetVideo will bail out if there is no primary window */
  ...
  if (_gdk_display->directfb->CreateSurface (_gdk_display->directfb,
         &dsc, &src) != DFB_OK)
  printf("_gdk_display->directfb->CreateSurface failed \n");
  impl->drawable.surface = src;
  /* <\lab126> */
  {
    DFBWindowDescription   desc;
    desc.flags = 0;
    ...
    desc.caps = 0;
    desc.surface_caps = DSCAPS_PRIMARY;
...
printf("in _gdk_windowing_window_init creating primary surface window - w=%d, h=%d\n", desc.width, desc.height);
  ...
  */
  // <lab126>
  // Taken out into gdkdisplay_directfb.c: gdk_display_open()
...
/* <lab126> */
IDirectFBSurface *
gdk_window_get_surface(GdkWindow *window) 
{
                                ...
                                GdkCursor *cursor)
{
  ...
  /* <lab126>: HUGE UNKNOWN: If the fflush is not added below, The cursor change is not performed. 
               Why a flush of stdout would affect the display of the cursor especially BEFORE it gets changed?
  */
  fflush(stdout);
  /* </lab126> */
                          ...
                          gint        y)
{
	...
	int status = layer->WarpCursor(layer, x, y);
    /* </lab126> */
}

/* <lab126> */
void
gdk_display_show_pointer (GdkDisplay *display,
                          gboolean    show)
{
    IDirectFBDisplayLayer *layer = _gdk_display->layer;
	layer->SetCooperativeLevel(layer, DLSCL_ADMINISTRATIVE);
	layer->EnableCursor(layer, show);
}
/*****************************************************************************/
                        ...
                        void                       *arg )
{
     ...
     char                *viewport_shift_env = getenv("DFB_VIEWPORT_Y_SHIFT");
     int dfb_shift = atoi(viewport_shift_env);
     /* Name */
     ...
     ret = dfb->GetDisplayLayer( dfb, id, &layer );
         if (ret) {
          ...
          layer->GetConfiguration(layer, &ret_config);
          printf("layer configw=%d, h=%d\n", ret_config.width, ret_config.height);
          if (arg) {
             int tmp = ret_config.width;
             ret_config.width = ret_config.height + dfb_shift;
             ret_config.height = tmp -dfb_shift ;
             ret_config.flags = (DFBDisplayLayerConfigFlags)(DLCONF_WIDTH | DLCONF_HEIGHT);

          /*  printf("setting layer config w=%d, h=%d\n", ret_config.width, ret_config.height);*/
             layer->SetCooperativeLevel( layer, DLSCL_ADMINISTRATIVE );
             layer->SetConfiguration(layer, &ret_config);
          }
          layer->GetSurface(layer, &ret_interface);
          ret_interface->GetSize(ret_interface, &w, &h);
                 ...
                 void                 *arg )
{
    ...
    /* printf( "screen w=%d, h=%d\n", w, h);*/
     ...
     /* Display layers */
   ret = screen->EnumDisplayLayers( screen, display_layer_callback, arg );
   if (ret)
...
void  gdk_set_orientation(int orientation, gboolean keyOnly)
{
    if (keyOnly) {
         gdk_directfb_set_orientation(orientation);
    } else {
        GdkScreen* screen = gdk_display_get_default_screen(_gdk_display);
        IDirectFB *directFB = _gdk_display->directfb;
        directFB->SetCooperativeLevel( directFB, DFSCL_NORMAL );
        directFB->SetVideoMode(directFB, gdk_screen_get_width   (screen),gdk_screen_get_height(screen), 8);
        directFB->EnumScreens( directFB, screen_callback, &orientation );
        gdk_directfb_set_orientation(orientation);
        directFB->EnumScreens( directFB, screen_callback, NULL );
    }
}
