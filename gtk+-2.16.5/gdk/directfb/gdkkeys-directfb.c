...
/**
 * is_five_way_key:
 * @keycode: a hardware keycode from DirectFB
 *
 * Returns: %TRUE if the specified keycode is a five way key.
 **/
gboolean
is_five_way_key (guint keycode)
{
	return ((keycode == 122) || /* CLASS_1_device UP */
                (keycode == 103) || /* CLASS_2_device UP */
                (keycode == 123) || /* CLASS_1_device DOWN */
                (keycode == 108) || /* CLASS_2_device DOWN */
                (keycode == 105) || /* LEFT (for both) */
                (keycode == 106) || /* RIGHT (for both) */
                (keycode == 92)  || /* CLASS_1_device SELECT */
                (keycode == 194));  /* CLASS_2_device SELECT */
}
/* </lab126> */
		...
		DFBInputDeviceKeySymbol     key_symbol)
{
	...
	if (key_id >= DIKI_KP_DIV && key_id <= DIKI_KP_9)
	{
		switch (key_symbol)
		{
			...
			case DIKS_BACK:          keyval = GDK_F11;         break;
	...
	else
	{
		switch (DFB_KEY_TYPE (key_symbol))
		{
			...
			case DIKT_FUNCTION:
				/* <lab126> */
                                if (key_symbol == 61719) { /* CLASS_1_device-only */
					keyval=GDK_Up;
					key_id = DIKI_UP;
				} else if (key_symbol == 61720) { /* CLASS_1_device-only */
					keyval=GDK_Down;
					key_id = DIKI_DOWN;
				} else if (key_symbol == 61713) {
					keyval= GDK_Select;
					key_id = DIKI_ENTER;
				} else {
...
/* <lab126> */
void gdk_directfb_set_orientation (int orientation)
{
                                  ...
                                  GdkEventKey    *event)
{
	...
	/* <lab126> */
	/**
	 * Implement sticky-shift behavior.
	 *
	 * Text keys - any key's key_symbol's DFB_KEY_TYPE is DIKT_UNICODE, which
	 * is meant for Unicode 3.x character, compatible to Latin-1, except for
	 * DIKS_NULL. SYM and Aa key' key_symbols are mapped to DIKS_NULL.
	 *
	 * If no text keys arrive between a shift key down and shift key up, the
	 * next text key will have sticky shift applied.
	 * This also implies that only a text key can clear a sticky shift.
	 *
	 * If a text key arrives between the shift key down and shift key up,
	 * the next key after the shift key up will NOT have sticky shift.
	 */
	static gboolean applyStickyShift = FALSE;

	gboolean isTextKey = (DFB_KEY_TYPE (dfb_event->key_symbol) == DIKT_UNICODE && dfb_event->key_symbol != DIKS_NULL);
	gboolean shiftBefore = (_gdk_directfb_modifiers & GDK_SHIFT_MASK) ? TRUE : FALSE;
	/**
	 * Do not convert modifiers for five way keys because five way
	 * keys come from a different device than all other keys. DirectFB
	 * does not apply modifiers across different devices, so the five way
	 * keys do not contain any modifiers. Therefore, a five way key should
	 * simply inherit the global modifier state, instead of set/reset such
	 * state.
	 */
	if (!is_five_way_key (dfb_event->key_code))
	{
		gdk_directfb_convert_modifiers (dfb_event->modifiers, dfb_event->locks);
	}
	gboolean shiftAfter = (_gdk_directfb_modifiers & GDK_SHIFT_MASK) ? TRUE : FALSE;

	if (!shiftBefore && shiftAfter)
	{
	    /* Got a new shift key down that is not in between a shift key down and
	       shift key up. Set sticky shift. */
	    applyStickyShift = TRUE;
	}
	else if (shiftBefore && shiftAfter && isTextKey)
	{
	    /* Got a text key between a shift key down and shift key up. Therefore,
	       clear the sticky shift. */
	    applyStickyShift = FALSE;
	}

        /**
         * On CLASS_2_device, the five-way-select's key_symbol is the same as
         * CLASS_1_device's five-way-down's key_symbol. So the code here turns
         * CLASS_2_device's five-way-select's dfb key_symbol into CLASS_1_device
         * five-way-select key_symbol based on the key code. This is so
         * that the call to gdk_directfb_translate_key/2 below can correctly
         * map both CLASS_2_device and CLASS_1_device five-way-select key to the
         * proper key value.
         */
        if (dfb_event->key_code == 194) /* CLASS_2_device five-way-select */
        {
                dfb_event->key_symbol = 61713;
        }
	/* </lab126> */
	...
	/* <lab126> */
	/* Sticky shift only applies to and is only cleared by a text key. */
	if (applyStickyShift && isTextKey)
	{
	    /* Clear sticky shift after being applied. */
	    applyStickyShift = FALSE;
	    /* Set the shift-mask for this event. */
	    event->state |= GDK_SHIFT_MASK;
	    /* Convert the dfb key_symbol to upper case. DFB_UPPER_CASE only
	       converts a key_symbol if the symbol is between 'a' and 'z'. */
	    dfb_event->key_symbol = DFB_UPPER_CASE (dfb_event->key_symbol);
	}
	/* </lab126> */
	...
	/* <lab126> */
        if ( keyOrientation == 2) { /* 90 degrees */
            switch (event->keyval) {
                case GDK_Up: {
                    event->keyval = GDK_Right;
                    dfb_event->key_id = DIKI_RIGHT;
                    event->hardware_keycode = 106;
                    break;
                }
                case GDK_Right: {
                    event->keyval = GDK_Down;
                    dfb_event->key_id = DIKI_DOWN;
                    event->hardware_keycode = 108;
                    break;
                }
                case GDK_Down: {
                    event->keyval = GDK_Left;
                    dfb_event->key_id = DIKI_LEFT;
                    event->hardware_keycode = 105;
                    break;
                }
                case GDK_Left: {
                    event->keyval = GDK_Up;
                    dfb_event->key_id = DIKI_UP;
                    event->hardware_keycode = 103;
                    break;
                }
            }
        } else if ( keyOrientation == 1) { /* 180 degrees */
            switch (event->keyval) {
                case GDK_Up: {
                    event->keyval = GDK_Down;
                    dfb_event->key_id = DIKI_DOWN;
