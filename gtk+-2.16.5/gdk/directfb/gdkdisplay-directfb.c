...
static int fd=-1;
void myDirtyRectHandler(const DFBRectangle *rect){
	u8 *fb, *vfb, *src,*dst;
	int fb_w,fb_h;
	int ret;
	int x,y;
	int x1=rect->x&0xfffe;
	...
	if(ret!=0) return;
	int diff;	
	unsigned int a,b,d=0; //for dithering
	#define NEXT_BYTE { a=*(src++);b=*(src++);\
                               *(dst++)=(a&0xf0)|(b>>4);}
	if(rect->w==fb_w) {
	   diff=rect->y*fb_w;
	   src=vfb+diff;
...
GdkDisplay * gdk_display_open (const gchar *display_name)
{
  ...
  // <lab126>
  // Change order because of initialization issue between _gdk_parent_root and colormap in LUT8 mode
  _gdk_windowing_window_init ();
  _gdk_windowing_image_init ();
  gdk_screen_set_default_colormap (_gdk_screen,
								   gdk_screen_get_system_colormap (_gdk_screen));
  // Copied from _gdk_windowing_window_init()
